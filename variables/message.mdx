...
Si el mensaje contiene propiedades adicionales (por ejemplo `length`, `mimeType`), accede a ellas con la misma notación de punto: `{{$message.mimeType}}`.

## Message en nodos de código

Dentro de un nodo de código accede a cada propiedad con `$message.get('propiedad', [valorPorDefecto])`:
